...
export default function HUDOverlay({ onMoonSelect }: HUDOverlayProps) {
  ...
  useEffect(() => {
    // Audio setup
    const audio = new Audio("/audios/Interstellar Main Theme - Extra Extended - Soundtrack by  Hans Zimmer.mp3");
    audio.loop = true;
    audioRef.current = audio;
    const playAudio = () => {
      if (audioRef.current?.paused) { // Play only if paused, to avoid interrupting if already playing
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (activeDropdown) {
        ...
        const isInsideAnyDropdown = Object.values(dropdownContainerRefs.current).some(
  ...
  const toggleMobileAccordion = (planetName: string) => {
    setActiveMobileAccordion(activeMobileAccordion === planetName ? null : planetName);
  };
  ...
  return (
    <>
      ...
      <nav
        className={`fixed top-0 left-0 w-full ${navBarHeightClass} bg-black/85 backdrop-blur-lg text-white z-30 hidden md:flex justify-center items-center space-x-2.5 px-6 shadow-lg`}
      >
        {navPlanets.map((planet) => (
          <div key={planet.name} className="relative" ref={el => dropdownContainerRefs.current[planet.name] = el}>
            <button
            ...
            >
              ...
              <ChevronDown size={18} className={`transition-transform duration-200 ${activeDropdown === planet.name ? 'rotate-180' : ''}`} />
            </button>
            <AnimatePresence>
      ...
      <div
        className={`fixed top-0 left-0 w-full ${navBarHeightClass} bg-black/85 backdrop-blur-lg text-white z-30 flex md:hidden justify-between items-center px-4 shadow-lg`}
      >
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              {isMobileNavOpen ? <XIcon size={28} strokeWidth={2.5}/> : <MenuIcon size={28} strokeWidth={2.5}/>}
            </motion.div>
          </AnimatePresence>
        </button>
      </div>
      ...
      <AnimatePresence>
        {isMobileNavOpen && (
          ...
          >
            <h2 className="text-xl font-bold text-blue-300 mb-4 px-1.5">Explore Sections</h2>
            <div className="flex-grow overflow-y-auto space-y-2 pr-0.5 scrollbar-thin scrollbar-thumb-gray-600/80 scrollbar-track-gray-700/50">
              {navPlanets.map((planet) => (
                <div key={planet.name} className="rounded-lg bg-gray-700/50 overflow-hidden">
                  <button
                  ...
                  >
                    <span>{planet.name}</span>
                    <ChevronDown size={22} className={`transition-transform duration-200 ${activeMobileAccordion === planet.name ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    ...
                    >
                      <div className="pt-1 pb-1.5 px-1.5 space-y-0.5">
                        {planet.moons?.map((moon) => (
                          <button
                            key={moon.name}
                            onClick={() => handleMobileMoonClick(moon, planet)}
